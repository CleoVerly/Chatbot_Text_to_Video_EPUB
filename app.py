...
def extract_text_from_epub(epub_path):
    ...
    try:
        # Menambahkan opsi untuk mengabaikan NCX yang rusak, sesuai peringatan
        book = epub.read_epub(epub_path, options={"ignore_ncx": True})
        items = list(book.get_items_of_type(ebooklib.ITEM_DOCUMENT))
        full_text = ""
        for item in items:
            # PERBAIKAN: Blok try-except yang lebih tangguh untuk menangani item yang rusak
            try:
                ...
                # Melewati item yang menyebabkan error dan melanjutkan ke item berikutnya
                continue
        return full_text
